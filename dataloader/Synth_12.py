...
class Synth_12_Dataset(torch.utils.data.Dataset):
    def __init__(self, root, train, test_size=0.1, transform=None):
        self.root = root
        self.train = train
        self.transform = transform

        self.data = np.load(os.path.join(root, "training.npy"), allow_pickle=True)
...
if __name__ == '__main__':
    ...
    train_dataset = Synth_12_Dataset(root=root_dir, train=True, test_size=0.1, transform=composed_transform)
    test_dataset = Synth_12_Dataset(root=root_dir, train=False, test_size=0.1, transform=composed_transform)
